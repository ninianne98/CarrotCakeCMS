...
namespace Carrotware.CMS.UI.Plugins.LoremIpsum.Code {
	...
	public class ContentCreator {
		...
		protected List<string> GetLines() {
			...
			var txt = WebHelper.ReadEmbededScript("Carrotware.CMS.UI.Plugins.LoremIpsum.Code.lorem.txt").Replace("\r\n", "\n");
		...
		protected List<string> GetWords() {
			var lines = GetLines();
			return (string.Join(" ", lines.Take(12).ToArray())).Replace(".", " ").Replace(",", " ")
		...
		protected string GetXWords(string line, int words) {
			return string.Join(" ", line.Replace(".", " ").Replace(",", " ").Replace("  ", " ").Replace("  ", " ")
						...
						.Take(words).Select(x => x.Trim()).ToArray());
		}
