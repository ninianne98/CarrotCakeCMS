		...
		<asp:Panel ID="pnlGallery" runat="server">
			...
			<div class="prettyphotoGallery gallery">
				<asp:Repeater ID="rpGallery" runat="server">
					<ItemTemplate>
						<a target="_blank" href="<%# String.Format("{0}", Eval("GalleryImage"))  %>" rel="prettyPhoto[<%=pnlGallery.ClientID %>]" title="<%# String.Format("{0}", Eval("GalleryImage"))  %>">
			...
			<asp:Panel ID="pnlScript" runat="server">
				<script type="text/javascript">
					$(document).ready(function () {
						InitPrettyPhoto("#<%=pnlGallery.ClientID %> a[rel^='prettyPhoto']", "<%=PrettyPhotoSkin %>");
					});
					/*
