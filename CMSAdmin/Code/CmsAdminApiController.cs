...
namespace Carrotware.CMS.UI.Admin {
	...
	public class CmsAdminApiController : ApiController {
		...
		[HttpGet]
		public HttpResponseMessage GetSiteAdminFolder() {
		...
		[HttpPost]
		public HttpResponseMessage RecordHeartbeat(ApiModel model) {
			try {
		...
		[HttpPost]
		public HttpResponseMessage CancelEditing(ApiModel model) {
			try {
		...
		[HttpPost]
		public HttpResponseMessage SendTrackbackPageBatch(ApiModel model) {
			try {
				string thisPage = model.ThisPage;
				_currentPageGuid = new Guid(thisPage);
				if (_currentPageGuid != Guid.Empty) {
		...
		[HttpPost]
		public HttpResponseMessage SendTrackbackBatch() {
			try {
		...
		[HttpPost]
		public HttpResponseMessage RecordEditorPosition(ApiModel model) {
			try {
		...
		[HttpGet]
		public HttpResponseMessage GetChildPages(string PageID, string CurrPageID) {
		...
		[HttpGet]
		public HttpResponseMessage GetPageCrumbs(string PageID, string CurrPageID) {
		...
		[HttpPost]
		public HttpResponseMessage UpdatePageTemplate(ApiModel model) {
			try {
		...
		[HttpPost]
		public HttpResponseMessage ValidateBlogFolders(ApiModel model) {
			try {
				...
				if (SiteData.CurrentSiteExists && !bDuplicate) {
					var exists = pageHelper.ExistingPagesBeginWith(SiteData.CurrentSite);
		...
		[HttpGet]
		public HttpResponseMessage FindUsers(string searchTerm) {
		...
		[HttpGet]
		public HttpResponseMessage FindCreditUsers(string searchTerm) {
			string search = CMSConfigHelper.DecodeBase64(searchTerm);
		...
		[HttpGet]
		public HttpResponseMessage ValidateUniqueCategory(string TheSlug, string ItemID) {
			try {
		...
		[HttpGet]
		public HttpResponseMessage ValidateUniqueTag(string TheSlug, string ItemID) {
			try {
				Guid CurrentItemGuid = new Guid(ItemID);
				var theSlug = CMSConfigHelper.DecodeBase64(TheSlug);
				theSlug = ContentPageHelper.ScrubSlug(theSlug);
		...
		[HttpPost]
		public HttpResponseMessage RecordSnippetHeartbeat(ApiModel model) {
			try {
				...
				if (item != null && !item.IsSnippetLocked()) {
		...
		[HttpPost]
		public HttpResponseMessage CancelSnippetEditing(ApiModel model) {
			try {
				Guid currentItemGuid = new Guid(model.ItemID);
				ContentSnippet item = GetSnippet(currentItemGuid);
		...
		[HttpGet]
		public HttpResponseMessage ValidateUniqueSnippet(string TheSlug, string ItemID) {
			try {
				Guid currentItemGuid = new Guid(ItemID);
				var theSlug = CMSConfigHelper.DecodeBase64(TheSlug);
				theSlug = ContentPageHelper.ScrubSlug(theSlug);
		...
		[HttpGet]
		public HttpResponseMessage GetSnippetVersionText(string DBKey) {
			try {
		...
		[HttpGet]
		public HttpResponseMessage GenerateNewFilename(string ThePageTitle, string GoLiveDate, string PageID, string Mode) {
			try {
				_currentPageGuid = new Guid(PageID);
		...
		[HttpGet]
		public HttpResponseMessage ValidateUniqueFilename(string TheFileName, string PageID) {
			try {
				_currentPageGuid = new Guid(PageID);
		...
		[HttpGet]
		public HttpResponseMessage GenerateBlogFilePrefix(string ThePageSlug, string GoLiveDate) {
			try {
				DateTime goLiveDate = Convert.ToDateTime(GoLiveDate);
		...
		[HttpGet]
		public HttpResponseMessage ValidateUniqueBlogFilename(string ThePageSlug, string GoLiveDate, string PageID) {
			try {
				_currentPageGuid = new Guid(PageID);
		...
		[HttpGet]
		public HttpResponseMessage GenerateCategoryTagSlug(string TheSlug, string ItemID, string Mode) {
			try {
				Guid currentItemGuid = new Guid(ItemID);
				var theSlug = CMSConfigHelper.DecodeBase64(TheSlug);
		...
		[HttpGet]
		public HttpResponseMessage GenerateSnippetSlug(string TheSlug) {
			try {
				var theSlug = TheSlug;
		...
		[HttpPost]
		public HttpResponseMessage MoveWidgetToNewZone(ApiModel model) {
			try {
		...
		[HttpPost]
		public HttpResponseMessage CacheWidgetUpdate(ApiModel model) {
			try {
		...
		[HttpGet]
		public HttpResponseMessage GetWidgetText(string DBKey, string ThisPage) {
			try {
				string dbKey = DBKey;
		...
		[HttpGet]
		public HttpResponseMessage GetWidgetVersionText(string DBKey, string ThisPage) {
			try {
				string dbKey = DBKey;
				string thisPage = ThisPage;
		...
		[HttpGet]
		public HttpResponseMessage GetWidgetLatestText(string DBKey, string ThisPage) {
			try {
				_currentPageGuid = new Guid(ThisPage);
				LoadGuids();
				Guid guidWidget = new Guid(DBKey);
		...
		[HttpPost]
		public HttpResponseMessage DeleteWidget(ApiModel model) {
			try {
		...
		[HttpPost]
		public HttpResponseMessage CopyWidget(ApiModel model) {
			try {
				string dbKey = model.DBKey;
				string thisPage = model.ThisPage;
		...
		[HttpPost]
		public HttpResponseMessage RemoveWidget(ApiModel model) {
			try {
				string dbKey = model.DBKey;
				string thisPage = model.ThisPage;
		...
		[HttpPost]
		public HttpResponseMessage ActivateWidget(ApiModel model) {
			try {
				string dbKey = model.DBKey;
				string thisPage = model.ThisPage;
		...
		[HttpPost]
		public HttpResponseMessage CacheGenericContent(ApiModel model) {
			try {
		...
		[HttpPost]
		public HttpResponseMessage CacheContentZoneText(ApiModel model) {
			try {
				string zoneText = model.ZoneText;
		...
		[HttpPost]
		public HttpResponseMessage PublishChanges(ApiModel model) {
			try {
				string thisPage = model.ThisPage;
		...
		protected HttpResponseMessage ReturnLongString(string jsonText) {
		...
		protected HttpResponseMessage GetStringContent(string val) {
			var response = new HttpResponseMessage(HttpStatusCode.OK);
			response.Content = new StringContent(val, Encoding.UTF8, "text/plain");
			return response;
		}

		protected HttpResponseMessage JsonSerialize<T>(List<T> val) {
			var serializer = new JavaScriptSerializer();
			return GetStringContent(serializer.Serialize(val));
		}

		protected HttpResponseMessage JsonSerialize(int val) {
			return JsonSerialize(val.ToString());
		}

		protected HttpResponseMessage JsonSerialize(string val) {
			var serializer = new JavaScriptSerializer();
			return GetStringContent(serializer.Serialize(val));
		}
