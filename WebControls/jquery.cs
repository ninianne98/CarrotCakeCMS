	...
	public class jquery : BaseWebControl {
		...
		public static string GeneralUri {
			get {
				if (string.IsNullOrEmpty(_generalUri)) {
					_generalUri = GetWebResourceUrl("jquery.jquery-1-11-3.js");
				}
		...
		protected override void RenderContents(HtmlTextWriter writer) {
			...
			switch (jqVer) {
				...
				case "3.7":
					jqVer = "3.7.1";
					sJQFile = GetWebResourceUrl("jquery.jquery-3-7-1.js");
					break;
				...
				case "2.2":
					jqVer = "2.2.4";
					sJQFile = GetWebResourceUrl("jquery.jquery-2-2-4.js");
					break;
				...
				case "1.11":
					jqVer = "1.11.3";
					sJQFile = GetWebResourceUrl("jquery.jquery-1-11-3.js");
					break;

				case "1.12":
					jqVer = "1.12.4";
					sJQFile = GetWebResourceUrl("jquery.jquery-1-12-4.js");
					break;

				case "1.10":
					jqVer = "1.10.2";
					sJQFile = GetWebResourceUrl("jquery.jquery-1-10-2.js");
					break;
				...
				case "1.9":
					jqVer = "1.9.1";
					sJQFile = GetWebResourceUrl("jquery.jquery-1-9-1.js");
					break;
			...
			writer.WriteLine("<!-- JQuery v. " + jqVer + " --> <script src=\"" + sJQFile + "\" type=\"text/javascript\"></script> ");
			...
			if (this.UseJqueryMigrate) {
				...
				if (jqVer.StartsWith("1.9") || jqVer.StartsWith("1.10") || jqVer.StartsWith("1.11")) {
					sJQFileMig = GetWebResourceUrl("jquery.jquery-mig-1-2-1.js");
				}

				if (jqVer.StartsWith("1.12") || jqVer.StartsWith("1.13")) {
					sJQFileMig = GetWebResourceUrl("jquery.jquery-mig-1-3-0.js");
				}

				if (jqVer.StartsWith("3")) {
					sJQFileMig = GetWebResourceUrl("jquery.jquery-mig-3-4-0.js");
				}

				if (!string.IsNullOrEmpty(sJQFileMig)) {
					writer.WriteLine("<!-- jQuery Migrate Plugin --> <script src=\"" + sJQFileMig + "\" type=\"text/javascript\"></script> ");
				}
			}

			string key = WebControlHelper.DateKey();

			writer.WriteLine("<!-- Carrot Helpers --> <script src=\"" + UrlPaths.HelperPath + "?ts=" + key + "\" type=\"text/javascript\"></script> ");
		}
	}
}